...
- **Live audio processing** with OpenAI's GPT-4o-mini-transcribe model
...
- **React 19.2** with Server Actions and latest features
...
Run the migration scripts in the `scripts/` folder in order to create the database schema. You can run these directly in the Supabase SQL Editor or use the Supabase CLI:
...
Displays real-time transcriptions with customization options. Tracks viewer engagement metrics including scroll behavior, active time, and visibility changes. Syncs with other viewers via Supabase Realtime.
...
5. Validated text saved to database and broadcast via WebSocket
6. Supabase Realtime notifies all connected viewers

### Viewer Flow
1. Viewer connects to event via slug
2. Loads historical transcriptions from database
3. Subscribes to Supabase Realtime for new transcriptions
4. Listens to WebSocket for interim updates
